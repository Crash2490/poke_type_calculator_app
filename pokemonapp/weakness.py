...
dfnd_agnst_typ_val_lst = {
    'normal': {'fighting': 2, 'normal': 1, 'flying': 1, 'poison': 1, 'ground': 1, 'rock': 1, 'bug': 1, 'steel': 1, 'fire': 1, 'water': 1, 'grass': 1, 'electric': 1, 'psychic': 1, 'ice': 1, 'dragon': 1, 'dark': 1, 'fairy': 1, 'ghost': 0},
...
def weakness(type1, type2):
    """Takes Pokemon and returns the values of opposing types."""

    if type2 != 'none':
        t1_dict = dfnd_agnst_typ_val_lst[type1]
        t2_dict = dfnd_agnst_typ_val_lst[type2]
        type_values = dual_type_calc(t1_dict, t2_dict)
        return type_values
    else:
        type_values = dfnd_agnst_typ_val_lst[type1]
        return type_values
